...
The hook runs after Jekyll finishes rendering all pages. It iterates through documents and injects the Twikoo container before the closing `</body>` tag.
...
If you're interested in implementing this on your own Chirpy blog, the complete plugin source code is available in my Git repositories. You can find it on [GitHub](https://github.com/exyone/exyone.is-a.dev) or any other platform where I host this blog. Look for `_plugins/twikoo-inject.rb`.
...
这个钩子在 Jekyll 完成所有页面渲染后运行。它遍历文档，在 `</body>` 标签之前注入 Twikoo 容器。
