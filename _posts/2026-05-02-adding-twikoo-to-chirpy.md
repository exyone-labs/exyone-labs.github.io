...
def inject_twikoo_comment(document)
  ...
  twikoo_container = <<~HTML
    ...
    <div id="twikoo-section" data-twikoo-injected="true" ...>
    ...
  HTML
...
## The Self-Exclusion Bug

> This one caught me completely off guard. The plugin was working on all posts—except this one.
{: .prompt-warning }

After fixing the build environment issues, I noticed something strange: Twikoo was injecting correctly into every post *except* this very article you're reading. I checked the generated HTML, cleared caches, rebuilt multiple times—nothing worked.

### The Root Cause

The problem? This article *contains the plugin's source code* in code blocks. When the plugin runs:

```ruby
return if output.include?('id="twikoo-section"')
```

It searches the entire rendered HTML output. Since this article demonstrates the plugin code, the string `id="twikoo-section"` appears in the code examples—so the plugin thinks it has already injected the Twikoo container and skips the injection!

This is a classic case of **self-exclusion**: the article's content triggers the plugin's "already done" check.

### The Fix

The solution is to use a marker that would never appear in article content:

```ruby
# Old: vulnerable to false positives
return if output.include?('id="twikoo-section"')

# New: unique marker that won't appear in code examples
return if output.include?('data-twikoo-injected="true"')
```

And add the corresponding attribute to the injected HTML:

```html
<div id="twikoo-section" data-twikoo-injected="true" style="...">
```

Using a `data-*` attribute serves two purposes:
1. It's semantically correct (HTML5 data attributes for custom data)
2. It's unique enough to avoid collisions with article content

> If you're writing technical documentation that includes your own plugin's code, always use markers that won't appear in code examples. A UUID or unique data attribute works well.
{: .prompt-tip }
...
| 2026-05-02 | Fixed self-exclusion bug by using `data-twikoo-injected` attribute as marker |
...
> 这个问题让我完全措手不及。插件在所有文章上都正常工作——除了这一篇。
{: .prompt-warning }

解决了构建环境问题后，我发现了一件奇怪的事：Twikoo 在每篇文章都能正确注入，*除了*你正在阅读的这篇。我检查了生成的 HTML、清理缓存、多次重新构建——都没用。

### 根本原因

问题出在哪？这篇文章的代码块中*包含了插件源码*。当插件运行时：

```ruby
return if output.include?('id="twikoo-section"')
```

它会搜索整个渲染后的 HTML 输出。由于这篇文章展示了插件代码，字符串 `id="twikoo-section"` 出现在代码示例中——于是插件认为它已经注入了 Twikoo 容器，直接跳过了注入！

这就是典型的**自我排除**问题：文章内容触发了插件的"已完成"检查。

### 修复方案

解决方案是使用一个永远不会出现在文章内容中的标记：

```ruby
# 旧方案：容易出现误判
return if output.include?('id="twikoo-section"')

# 新方案：独特的标记，不会出现在代码示例中
return if output.include?('data-twikoo-injected="true"')
```
...
使用 `data-*` 属性有两个好处：
1. 语义正确（HTML5 data 属性用于自定义数据）
2. 足够独特，不会与文章内容冲突

> 如果你在写技术文档时包含了自己插件的代码，一定要使用不会出现在代码示例中的标记。UUID 或独特的 data 属性都是不错的选择。
{: .prompt-tip }
...
| 2026-05-02 | 修复自我排除 Bug，使用 `data-twikoo-injected` 属性作为标记 |
